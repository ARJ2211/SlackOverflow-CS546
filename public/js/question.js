...
const handleUpdateQuestionModal = () => {
    ...
    if (modal) {
        if (modal.classList.contains('hidden')) {
            ...
            const handle = document.getElementById("updateQuestionResizeHandle");
            let valueY, valueH;
            handle.onmousedown = e => {
                valueY = e.clientY;
                valueH = editor.offsetHeight;
                document.onmousemove = e => {
                    editor.style.height = (valueH + (valueY - e.clientY)) < 32 ? valueH : (valueH + (valueY - e.clientY)) + "px";
                    updateQuestionQuill.root.style.minHeight = editor.style.height
                };
                document.onmouseup = () => document.onmousemove = null;
            };
        } else {
...
const addLabels = (labelsArray) => {
    ...
    labelsArray.forEach(label => {
        ...
        labelTag.className = "text-sm border border-white font-medium rounded-3xl pl-2 pr-1 text-white bg-[#F0BD66] flex gap-1";
