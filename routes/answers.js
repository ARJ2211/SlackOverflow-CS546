...
router.route("/").post(async (req, res) => {
    ...
    try {
        question_id = validator.isValidMongoId(
            req.body.question_id,
            "question_id"
        );
        answer = validator.isValidString(req.body.answer, "answer");
        answer_delta = validator.isValidString(
            req.body.answer_delta,
            "answer_delta"
        );
        answer_content = validator.isValidString(
            req.body.answer_content,
            "answer_content"
        );
        user_id = validator.isValidMongoId(req.body.user_id, "user_id");
        is_accepted = Boolean(req.body.is_accepted);
        question_status = validator.isValidString(
            req.body.question_status,
            "question_status"
        );

        answer = xss(answer);
        answer_delta = xss(answer_delta);
        answer_content = xss(answer_content);
    } catch (e) {
        e.status = 400;
    ...
    try {
        ...
        return res
            .status(200)
            .json({ message: "Answer added to the question" });
    } catch (e) {
        e.status = 404;
        return handleError(res, e);
    }
    ...
    .patch(async (req, res) => {
        ...
        try {
            ...
            if (
                ...
                typeof is_accepted === "boolean" &&
                is_accepted != null
            ) {
                is_accepted = validator.isValidBoolean(
                    is_accepted,
                    "is_accepted"
                );
            }
        ...
        try {
            await answersData.updateAnswer(answer_id, {
                answer,
                answer_delta,
                answer_content,
                user_id,
            ...
            if (
                ...
                await questionsData.updateAcceptedAnswerId(
                    question_id,
                    answer_id,
                    is_accepted
                );
