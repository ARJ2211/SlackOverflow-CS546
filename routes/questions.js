...
router
    ...
    .patch(async (req, res) => {
        ...
        let { question, question_delta, question_content, user_id, labels } =
            req.body;
        ...
        try {
            ...
            if (question_content) {
                question_content = validator.isValidString(
                    question_content,
                    "question_content"
                );
            }
            ...
            if (question_delta) {
                question_delta = validator.isValidString(
                    question_delta,
                    "question_delta"
                );
            }
            ...
            if (labels && Array.isArray(labels) && labels.length > 0) {
                labels = labels.map((label) =>
                    validator.isValidMongoId(label, "label")
                );
            }
        ...
        try {
            ...
            if (question_delta !== undefined)
                updatePayload.question_delta = question_delta;
            if (question_content !== undefined)
                updatePayload.question_content = question_content;
            if (labels !== undefined) updatePayload.labels = labels;
            // we are NOT updating user_id from client here

            await questionsData.updateQuestion(
                { _id: question_id },
                updatePayload
            );
...
router.route("/:id/votes").patch(async (req, res) => {
    let user_id = req.body.user_id;
    let question_id = req.params.id;
    ...
    try {
        const updatedVotes = await questionsData.updateUpVotes(
            question_id,
            user_id
        );
...
router.route("/:id/status").patch(async (req, res) => {
    ...
    try {
        const updatedStatus = await questionsData.updateStatus(
            question_id,
            status
        );
